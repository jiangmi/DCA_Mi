...
#ifndef DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DDS_MODEL_HPP
#define DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DDS_MODEL_HPP
...
template <typename point_group_type>
double* dds_model<point_group_type>::initialize_r_DCA_basis() {
...
template <typename point_group_type>
double* dds_model<point_group_type>::initialize_k_DCA_basis() {
...
template <typename point_group_type>
double* dds_model<point_group_type>::initialize_r_LDA_basis() {
...
template <typename point_group_type>
double* dds_model<point_group_type>::initialize_k_LDA_basis() {
...
template <typename point_group_type>
std::vector<int> dds_model<point_group_type>::get_flavors() {
...
template <typename point_group_type>
std::vector<std::vector<double>> dds_model<point_group_type>::get_a_vectors() {
...
template <typename point_group_type>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> dds_model<
...
template <typename point_group_type>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void dds_model<point_group_type>::initialize_H_interaction(
    func::function<double, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                              func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
  if (BandDmn::dmn_size() != BANDS)
    throw std::logic_error("dds_model has 3 bands.");
...
template <typename point_group_type>
template <class domain>
void dds_model<point_group_type>::initialize_H_symmetry(func::function<int, domain>& H_symmetries) {
  H_symmetries = -1;
...
template <typename point_group_type>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void dds_model<point_group_type>::initialize_H_0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
  ...
  // 0, 1, 2 denotes dx2, dz2, s orbitals
  for (int k_ind = 0; k_ind < KDmn::dmn_size(); ++k_ind) {
    ...
    for (int s = 0; s < 2; s++) {
        ...
        H_0(1, s, 1, s, k_ind) = val2;
        H_0(2, s, 2, s, k_ind) = vals;
...
#endif  // DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DDS_MODEL_HPP
