...
// Author: Mi Jiang
//
// Three-band model coupled to one layer, different from dpsNi_model that has two s layers
// 260106Peng: bilayer d-p-s model

#ifndef DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DPS_3D_model_LATTICE_HPP
#define DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DPS_3D_model_LATTICE_HPP
...
double* dps_3D_model<point_group_type>::initialize_r_DCA_basis() {
...
template <typename point_group_type>
double* dps_3D_model<point_group_type>::initialize_k_DCA_basis() {
...
template <typename point_group_type>
double* dps_3D_model<point_group_type>::initialize_r_LDA_basis() {
...
template <typename point_group_type>
double* dps_3D_model<point_group_type>::initialize_k_LDA_basis() {
...
template <typename point_group_type>
std::vector<int> dps_3D_model<point_group_type>::get_flavors() {
...
template <typename point_group_type>
std::vector<std::vector<double>> dps_3D_model<point_group_type>::get_a_vectors() {
...
template <typename point_group_type>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> dps_3D_model<
...
template <typename point_group_type>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void dps_3D_model<point_group_type>::initialize_H_interaction(
    func::function<double, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                              func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
  if (BandDmn::dmn_size() != BANDS)
    throw std::logic_error("dps 3D model has eight bands.");
...
template <typename point_group_type>
template <class domain>
void dps_3D_model<point_group_type>::initialize_H_symmetry(
...
template <typename point_group_type>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void dps_3D_model<point_group_type>::initialize_H_0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
  ...
  const auto t_ps = parameters.get_t_ps();

  const auto ep_d = parameters.get_ep_d();
  const auto ep_p = parameters.get_ep_p();
  const auto es   = parameters.get_es();

  const auto t_ss100 = parameters.get_t_ss100();
  const auto t_ss110 = parameters.get_t_ss110();
  const auto t_ss101 = parameters.get_t_ss101();
  const auto t_ss001 = parameters.get_t_ss001();
  const auto t_ss111 = parameters.get_t_ss111();
  ...
  for (int k_ind = 0; k_ind < KDmn::dmn_size(); ++k_ind) {
    ...
    const auto valspx =  2. * I * t_ps * std::sin(k[0] / 2.);
    const auto valspy = -2. * I * t_ps * std::sin(k[1] / 2.);

    const auto valss100 = 2. * t_ss100 * (std::cos(k[0]) + std::cos(k[1]));
    const auto valss110 = 4. * t_ss110 * std::cos(k[0]) * std::cos(k[1]);
    const auto valss001 = t_ss001;
    const auto valss101 = 2. * t_ss101 * (std::cos(k[0]) + std::cos(k[1]));
    const auto valss111 = 4. * t_ss111 * std::cos(k[0]) * std::cos(k[1]);
    ...
    for (int s = 0; s < 2; s++) {
      ...
      H_0(3, s, 3, s, k_ind) = es + valss100 + valss110;

      H_0(4, s, 4, s, k_ind) = ep_d;
      H_0(5, s, 5, s, k_ind) = ep_p;
      H_0(6, s, 6, s, k_ind) = ep_p;
      H_0(7, s, 7, s, k_ind) = es + valss100 + valss110;

      H_0(3, s, 7, s, k_ind) = valss001 + valss101 + valss111;
      H_0(7, s, 3, s, k_ind) = valss001 + valss101 + valss111;

      H_0(0, s, 1, s, k_ind) =  valdpx;
      H_0(1, s, 0, s, k_ind) = -valdpx;
      H_0(4, s, 5, s, k_ind) =  valdpx;
      H_0(5, s, 4, s, k_ind) = -valdpx;

      H_0(0, s, 2, s, k_ind) =  valdpy;
      H_0(2, s, 0, s, k_ind) = -valdpy;
      H_0(4, s, 6, s, k_ind) =  valdpy;
      H_0(6, s, 4, s, k_ind) = -valdpy;

      H_0(1, s, 2, s, k_ind) = valpxpy;
      H_0(2, s, 1, s, k_ind) = valpxpy;
      H_0(5, s, 6, s, k_ind) = valpxpy;
      H_0(6, s, 5, s, k_ind) = valpxpy;

      // below for p-s hybridization:
      // similar to d3z2r2 has +,-,+ sign structure so that it is - in x-y plane
      H_0(3, s, 1, s, k_ind) = valspx;
      H_0(1, s, 3, s, k_ind) =  -valspx;
      H_0(3, s, 2, s, k_ind) =  -valspy;
      H_0(2, s, 3, s, k_ind) = valspy;

      H_0(7, s, 5, s, k_ind) = valspx;
      H_0(5, s, 7, s, k_ind) =  -valspx;
      H_0(7, s, 6, s, k_ind) =  -valspy;
      H_0(6, s, 7, s, k_ind) = valspy;

      H_0(7, s, 1, s, k_ind) = valspx;
      H_0(1, s, 7, s, k_ind) =  -valspx;
      H_0(7, s, 2, s, k_ind) =  -valspy;
      H_0(2, s, 7, s, k_ind) = valspy;

      H_0(3, s, 5, s, k_ind) = valspx;
      H_0(5, s, 3, s, k_ind) =  -valspx;
      H_0(3, s, 6, s, k_ind) =  -valspy;
      H_0(6, s, 3, s, k_ind) = valspy;
    }
  }
}
...
#endif  // DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_DPS_3D_model_LATTICE_HPP
