...
double* trilayer_lattice<point_group_type>::initialize_r_DCA_basis() {
...
template <typename point_group_type>
double* trilayer_lattice<point_group_type>::initialize_k_DCA_basis() {
...
template <typename point_group_type>
double* trilayer_lattice<point_group_type>::initialize_r_LDA_basis() {
...
template <typename point_group_type>
double* trilayer_lattice<point_group_type>::initialize_k_LDA_basis() {
...
template <typename point_group_type>
std::vector<int> trilayer_lattice<point_group_type>::get_flavors() {
...
template <typename point_group_type>
std::vector<std::vector<double>> trilayer_lattice<point_group_type>::get_a_vectors() {
...
template <typename point_group_type>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> trilayer_lattice<
...
template <typename point_group_type>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void trilayer_lattice<point_group_type>::initialize_H_interaction(
    func::function<double, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                              func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
  if (BandDmn::dmn_size() != BANDS)
    throw std::logic_error("Trilayer lattice has three bands.");
...
template <typename point_group_type>
template <class domain>
void trilayer_lattice<point_group_type>::initialize_H_symmetry(func::function<int, domain>& H_symmetries) {
  H_symmetries = -1;
...
template <typename point_group_type>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void trilayer_lattice<point_group_type>::initialize_H_0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
...
#endif  // DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_TRILAYER_LATTICE_HPP
